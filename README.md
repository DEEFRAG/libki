...
setup_libki_kiosk_lockdown.sh - Ein Script welches Linux abhärtet, damit Anwender nicht mehr einfach so den Libki Client minimieren können und so weiter.

install_libki_server.sh - Installiert den Libki Server auf Debian 13.5 oder 13.6 in einem Docker Container und führt diesen dann aus.

add_german.sh - Fügt die deutsche Sprachdatei de.po zum Libki Server hinzu und wendet api_key_template.patch an um fehlende Übersetzungen hinzuzufügen.
...
libki-server-531-de.zip - Dieses Archiv enthält die Quelldateien für den Libki Server Version 5.3.1 inklusive der deutschen Sprachdateien für Linux.
